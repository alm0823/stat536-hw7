...
df_all$deltaAIC <- round(as.numeric(as.character(df_all$AIC)), 2) - round(as.numeric(as.character(df_all$AIC[1])), 2)
...
<<prob4>>=
par(mfrow=c(1,1))
plot(mex$value~time(mex$year), type = "l", ylab = expression(CO[2]))
text(1, at = c(seq(0,80,by=13.33)), labels = c(seq(min(mex$year), max(mex$year), by = 1)), str = 45, pos = 2)
