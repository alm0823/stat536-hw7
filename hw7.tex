...
Linear Year, 5th Order Harmonic & 12 & 202.38 & 0.00 \\ 
  ...
  Quadratic Year, Seasonal Means & 14 & 205.07 & 2.70 \\ 
  ...
  No Year, Seasonal Means & 12 & 513.61 & 311.23 \\ 
   \hline
\end{tabular}
\end{table}
% latex table generated in R 3.3.1 by xtable 1.8-2 package
...
All the models we considered are listed below. The top model had the lowest AIC, the Linear Year, 5th Order Harmonic with 12 free degrees of freedom, but it is not strongly supported over the Linear Year, Seasonal Means model. However, all of the models with a linear or quadratic trend and seasonal means or 5th order harmonics are better supported over the other eight models.
...
\hlstd{nt.sm} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlstr{"No Year, Seasonal Means"}\hlstd{, df_nt.sm, aic_nt.sm)}
...
\hlstd{lt.sm} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlstr{"Linear Year, Seasonal Means"}\hlstd{, df_lt.sm, aic_lt.sm)}
...
\hlstd{qt.sm} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlstr{"Quadratic Year, Seasonal Means"}\hlstd{, df_qt.sm, aic_qt.sm)}
...
\hlcom{#discuss: what does m do?  Ho: the number of cycles}
    ...
    \hlcom{# Kenny's pvalue = 0.12 (number of terms is related to number of cycles)}
...
\hlstd{mod_lt.sh5} \hlkwb{<-} \hlkwd{paste}\hlstd{(}\hlstr{"$y_\{t\} = \textbackslash{}\textbackslash{}gamma_\{0\} + \textbackslash{}\textbackslash{}gamma_\{1\}year + \textbackslash{}\textbackslash{}Sigma_\{i=1:5\} 
                    \textbackslash{}\textbackslash{}beta_\{i\}cos(2i\textbackslash{}\textbackslash{}pi f(t)) + \textbackslash{}\textbackslash{}psi_\{i\}sin(2i\textbackslash{}\textbackslash{}pi f(t)) + e_\{t\}$"}\hlstd{)}
...
\hlstd{mod_qt.sh5} \hlkwb{<-} \hlkwd{paste}\hlstd{(}\hlstr{"$y_\{t\} = \textbackslash{}\textbackslash{}gamma_\{0\} + \textbackslash{}\textbackslash{}gamma_\{1\}year + \textbackslash{}\textbackslash{}gamma_\{1\}year^\{2\} + 
                    \textbackslash{}\textbackslash{}Sigma_\{i=1:5\} \textbackslash{}\textbackslash{}beta_\{i\}cos(2i\textbackslash{}\textbackslash{}pi f(t)) + \textbackslash{}\textbackslash{}psi_\{i\}sin(2i\textbackslash{}\textbackslash{}pi f(t)) + e_\{t\}$"}\hlstd{)}
...
\hlcom{#print(xtable(all_mod), sanitize.text.function = function(x) x)}
